// ppxsolid.cpp : Implementation of Cppxsolid

#include "stdafx.h"
#include "ppxsolid.h"
#include "SwDocument.h"
#include "CppxsolidDialog.h"
#include <string>
#include <list>
...
// Cppxsolid
void Cppxsolid::AddCommandManager()
{
	...
	HRESULT retVal = iCmdMgr->IGetGroupDataFromRegistry(MAIN_CMD_GROUP_ID, count, registryIDs, &getDataResult);
		long knownIDs[2] = {MAIN_ITEM_ID1, MAIN_ITEM_ID2};

	if(getDataResult)
	{
		if(!CompareIDs(registryIDs, count, knownIDs, 2)) // if the ID's don't match, reset the command group
		{
			ignorePrevious = true;
		}
	}
	...
	tip.LoadString(IDS_TOOLBAR_PMPTIP);
	hint.LoadString(IDS_TOOLBAR_PMPHINT);
	hret = icmdGroup->AddCommandItem2(tip, -1, hint, tip, 0, callback, enable, MAIN_ITEM_ID2, menuToolbarOption, &cmdIndex1);
...
void Cppxsolid::RemoveCommandManager()
{
	iBmp->Dispose();
	iBmp->Release();
...
bool Cppxsolid::CompareIDs(long * storedIDs, long storedSize, long * addinIDs, long addinSize)
{
...
void Cppxsolid::RemovePMP()
{
	userPropertyPage->Destroy();
	userPropertyPage = NULL;
}

STDMETHODIMP Cppxsolid::OnDocChange(void)
{
...
STDMETHODIMP Cppxsolid::OnFileOpenPostNotify(BSTR fileName)
{
	HRESULT hres = S_OK;
...
HRESULT Cppxsolid::AttachEventsToAllDocuments()
{
	VARIANT_BOOL attached = VARIANT_TRUE;
	USES_CONVERSION;
	...
	while (iModelDoc2 != NULL)
	{
			...
			{
				ATLTRACE("\tCppxs::DocumentLoadNotify current part not found\n");
...
VARIANT_BOOL Cppxsolid::AttachModelEventHandler(CComPtr<IModelDoc2> iModelDoc2)
{
	VARIANT_BOOL attached = VARIANT_TRUE;
	if (iModelDoc2 == NULL)
		return VARIANT_FALSE;
	...
	if (iter == openDocs.end())
	{
		...
		pDoc->Init((CComObject<Cppxsolid>*)this, iModelDoc2);
...
VARIANT_BOOL Cppxsolid::DetachEventHandlers()
{
...
VARIANT_BOOL Cppxsolid::DetachModelEventHandler(IUnknown *iUnk)
{
	VARIANT_BOOL detached = VARIANT_TRUE;
	TMapIUnknownToDocument::iterator iter;
...
STDMETHODIMP Cppxsolid::OnModelDocChange(void)
{
	// TODO: Add your implementation code here
	return S_OK;
}
...
STDMETHODIMP Cppxsolid::OnFileNew(LPDISPATCH newDoc, long docType, BSTR templateName)
{
	HRESULT hres = S_OK;
	hres = AttachEventsToAllDocuments();
   return hres;
...
STDMETHODIMP Cppxsolid::ConnectToSW(LPDISPATCH ThisSW, long Cookie, VARIANT_BOOL * IsConnected)
{
	...
	iSwApp->SetAddinCallbackInfo((long)_AtlBaseModule.GetModuleInstance(), static_cast<Ippxsolid*>(this), addinID, &status);
...
STDMETHODIMP Cppxsolid::DisconnectFromSW(VARIANT_BOOL * IsDisconnected)
{
...
STDMETHODIMP Cppxsolid::ToolbarCallback0(void)
{
	// TODO: Add your implementation code here
...
STDMETHODIMP Cppxsolid::ToolbarEnable0(long* status)
{
	// TODO: Add your implementation code here
	*status = 1;
	return S_OK;
...
STDMETHODIMP Cppxsolid::FlyoutCallback(void)
{
	CComBSTR tip;
	CComBSTR callback;
	CComBSTR enable;
...
STDMETHODIMP Cppxsolid::FlyoutCallback1(void)
{
	// TODO: Add your implementation code here
...
STDMETHODIMP Cppxsolid::FlyoutEnable0(long* status)
{
	// TODO: Add your implementation code here
	*status = 1;
	return S_OK;
...
STDMETHODIMP Cppxsolid::FlyoutEnableCallback0(long* status)
{
	// TODO: Add your implementation code here
	*status = 1;
	return S_OK;
...
STDMETHODIMP Cppxsolid::EnablePMP(long* status)
{
	*status = 0;
	if (userPropertyPage != NULL)
	{
