...
void CSwDocument::Init(Cppxsolid* addinPtr, IModelDoc2* modDoc)
{
	userAddin = addinPtr;
...
VARIANT_BOOL CSwDocument::AttachModelViewEventHandlers()
{
	...
	// During ModelDocChange, new active doc is not visible yet.
	// Avoid adding a new CDocView if document just went from visible => non-visible.
	if (bIsVisible != VARIANT_FALSE)
	{
		if (pUnk1 == pUnk2)
		{
